...
- (void)didLoadFromCCB {
    ...
    //SpriteBuilder, at the moment, does not have a Parallax Node, so we need to manually
    //take each of the clouds and bushes and switch them to the children of the CCParallaxNode we just created using code
...
- (void)update:(CCTime)delta
{
    ...
    //moving the whole parallax background (which moves its children all at once). We just change the offset of the clouds and bushes only if the parallax background's motion moves one of the instances all the way off the screen such that it needs to be looped back around.
